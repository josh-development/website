						...
						setMembers((old) => [
							...
							...(data as GithubMembers)
								.filter(
									(member) =>
										member.type !== "Bot" &&
										!old.some((oldMember) => oldMember.id === member.id),
								)
								.map((member) => ({
									login: member.login,
									id: member.id,
			...
			<div class="flex">
				<div>
					...
					<p class="dark:text-gray-300 mt-2">
						Copyright &copy; {new Date().getFullYear()} - Josh Team
					</p>
				</div>
				<div class="ml-auto">
					<div class="flex items-center flex-wrap mt-2 -mr-2 justify-center">
						{shuffledMembers().map((member) => (
							<a
								...
								target="_blank"
								class="focus:outline-none w-12 h-12 bg-cover bg-center rounded-md -ml-2"
							>
								<img
									loading="lazy"
