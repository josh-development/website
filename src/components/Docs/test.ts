// This stinky code is just for refrence
...
import {
	RepositoryContentFileType,
	type RepositoryContent
} from './types/RepositoryContent';
...
const references: Record<string, string> = {
	...
	'__global.NodeJS.Timeout':
		'https://nodejs.org/api/timers.html#timers_class_timeout',
	'global.NodeJS.Timer':
		'https://nodejs.org/api/timers.html#timers_class_timeout',
	'__global.NodeJS.Timer':
		'https://nodejs.org/api/timers.html#timers_class_timeout',
...
ReferenceTypeParser.formatToString = (options) => {
	...
	const typeArguments =
		parser.typeArguments.length > 0
			? `<${parser.typeArguments
					.map((type) => (project ? type.toString(project) : type.toString()))
					.join(', ')}\\>`
			: '';
	...
	if (parser.id && parser.id > 0 && project) {
		...
		if (result) {
			if (result instanceof TypeParameterParser)
			...
			if ('external' in result && !result.external) {
				...
				if (result instanceof NamespaceParser) {
					return `[\`${parser.name}\`](../${parents
						...
						.map(
							(parent) =>
								`namespace/${parent.name.toLowerCase().replace(/\s/g, '-')}/`
						)
						.join('/')}/namespace/${parser.name
						.toLowerCase()
						.replace(/\s/g, '-')}.mdx)${typeArguments}`;
			...
			console.warn(
				yellow(
					`${bold('[WARN]')} Unable to find parser for ${parser.name} (${
						parser.id
					})`
				)
			);
	...
	if (parser.packageName) {
		if (parser.packageName === 'discord.js') {
			return `[\`${
				parser.name
			}\`](https://discord.js.org/#/docs/discord.js/main/search?${new URLSearchParams(
				{
					query: parser.name
				}
			)})${typeArguments}`;
			...
			return `[\`${
				parser.name
			}\`](https://discord.js.org/#/docs/collection/main/search?${new URLSearchParams(
				{
					query: parser.name
				}
			)})${typeArguments}`;
		} else if (parser.packageName.startsWith('@sapphire/')) {
			const filteredNeighbourProjects: [string | null, ProjectParser][] = [
				...neighbourProjects
			]
				...
				.map(([, [group, projectParsers]]) => {
					...
					const [latestVersion] = findLatestVersions(
						projectParsers.map((projectParser) => projectParser.version!)
					);

					return [
						group,
						projectParsers.find(
							(projectParser) => projectParser.version === latestVersion
						)!
					];
				});
			...
			if (neighbourProject) {
				...
				if (result) {
					const parents = recursivelyGetNamespaceParents(
						result,
						neighbourProject
					);
					...
					if (result instanceof NamespaceParser) {
						return `[\`${parser.name}\`](../../../${
							group ? `${group}/` : ''
						}${parser.packageName.replace('@sapphire/', '')}/${
		...
		for (const [ref, url] of Object.entries(references)) {
			if (ref === parser.name)
				return `[\`${parser.name}\`](${url})${typeArguments}`;
		}
...
export async function docusaurusTypeDocJsonParser(options: PluginOptions) {
	...
	console.info(
		blue(
			`${bold('[INFO]')} Fetching repository content... ${bold(
				githubContentUrl
			)}`
		)
	);

	const repositoryContents = await fetch<RepositoryContent[]>(
		githubContentUrl,
		{ headers }
	);
	const repositoryDirectories = repositoryContents.filter(
	...
	for (const directory of repositoryDirectories) {
		...
		const directoryContents = await fetch<RepositoryContent[]>(directory.url, {
			headers
		});
		const latestVersions = findLatestVersions(
			directoryContents
				...
				.map((directoryContent) =>
					directoryContent.name.replace('.json', '').replace('v', '')
				)
		);

		for (const directoryContent of directoryContents) {
			if (directoryContent.type === RepositoryContentFileType.Directory) {
				const subDirectoryContents = await fetch<RepositoryContent[]>(
					directoryContent.url,
					{ headers }
				);

				writeCategoryYaml(
					resolve(siteDir, 'docs', 'Documentation', directory.name),
					'',
					directory.name,
					0
				);
				...
				const latestVersions = findLatestVersions(
					subDirectoryContents
						.filter(
							(subDirectoryContent) => subDirectoryContent.name !== 'main.json'
						)
						.map((subDirectoryContent) =>
							subDirectoryContent.name.replace('.json', '').replace('v', '')
						)
				);
				const filteredSubDirectoryContents = subDirectoryContents.filter(
					(subDirectoryContent) =>
						[...latestVersions, 'main'].includes(
							subDirectoryContent.name.replace('.json', '').replace('v', '')
						)
				);
				...
				for (const subDirectoryContent of filteredSubDirectoryContents) {
					...
					const data = await fetch<ProjectParser.Json>(
						subDirectoryContent.download_url
					);
					const incomingTypeDocJsonParserVersion = data.typeDocJsonParserVersion
						.split('.')
					...
					if (
						incomingTypeDocJsonParserVersion[0] !==
						currentTypeDocJsonParserVersion[0]
					)
						continue;

					const projectParser = new ProjectParser({
						data,
						version: subDirectoryContent.name.replace('.json', '')
					});

					if (
						directoryContents.length > 1 &&
						subDirectoryContent.name !== 'main.json'
					) {
						neighbourProjects.set(projectParser.name, [
							directoryContent.name,
							[projectParser]
						]);
					} else {
						if (neighbourProjects.has(projectParser.name)) {
							const [, neighbourProject] = neighbourProjects.get(
								projectParser.name
							)!;
				...
				if (directoryContent.download_url === null)
					throw new Error(
						`The 'download_url' field is null for '${directory.name}/${directoryContent.name}'`
					);
				...
				const outputDir = resolve(
					...
					'Documentation',
					directory.name,
					directoryContent.name.replace('.json', '')
				);
				...
				const projectParser = new ProjectParser({
					data,
					version: directoryContent.name.replace('.json', '')
				});

				if (
					directoryContents.length > 1 &&
					directoryContent.name !== 'main.json'
				) {
				...
				if (parsed.has(projectParser.name)) {
					...
					existingProjectParsers.push(projectParser);
					parsed.set(projectParser.name, [
						outputDir,
						existingProjectParsers,
						false
					]);
				}
...
function getNamespaceParent(
	result: SearchResult,
	project: ProjectParser
): NamespaceParser | null {
	return 'namespaceParentId' in result && result.namespaceParentId
		? (project.find(result.namespaceParentId) as NamespaceParser | null)
		: null;
}

function recursivelyGetNamespaceParents(
	result: SearchResult,
	project: ProjectParser
): NamespaceParser[] {
