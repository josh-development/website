...
export default function HomeSubtitle() {
	...
	const [subtitle, setSubtitle] = createSignal(
		subtitlePackageList[subtitleIndex()],
	);
	...
	return (
		<a
			class="flex font-maven dark:text-white"
			target="_blank"
			href={`https://npmjs.org/package/@joshdb/${
				subtitlePackageList[subtitleIndex()]
			}`}
		...
		>
			<span>
				@joshdb/
				<span class="text-josh">{subtitle()}</span>
			</span>
			<span class="h-5 mt-1.5 bg-zinc-800 dark:bg-gray-300 animate-cursor w-0.5 ml-0.5" />
		</a>
	);
}
