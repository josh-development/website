...
export const quickStart = `import { Josh } from '@joshdb/core';

const josh = new Josh({ name: 'users' });

await josh.init();`;
export const typescript = `import { Josh } from '@joshdb/core';
...
<CodeBlock lang="ts" code={quickStart} />
...
<CodeBlock lang="ts" code={typescript} />